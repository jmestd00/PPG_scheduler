...
public class PPGScheduler {
    ...
    public PPGScheduler() throws PPGSchedulerException {
        ...
        // LocalDate fecha = LocalDate.of(año, mes, día);
        diluidores.add(new Diluidor(1, "Diluidor 1", 50, LocalDate.of(2023, 1, 1)));
        diluidores.add(new Diluidor(2, "Diluidor 2", 100, LocalDate.of(2023, 1, 1)));
        diluidores.add(new Diluidor(3, "Diluidor 3", 150, LocalDate.of(2023, 1, 1)));
        // diluidores.add(new Diluidor(4, "Diluidor 4", 80, LocalDate.of(2023, 1, 1)));
        // diluidores.add(new Diluidor(5, "Diluidor 5", 90, LocalDate.of(2023, 1, 1)));

        // diluidores.add(new Diluidor(3,"",150, LocalDate.of(2023, 1, 1)));
    }
        ...
        for (int i = 0; i <= toleranciaRetraso; i++) {
            for (Diluidor diluidor : diluidores) {
                ...
                if ((fechaFinTemp.isBefore(fechaNecesidad.plusDays(i)) || fechaNecesidad.equals(fechaFinTemp)) &&
                        diluidor.getCapacity() >= loteActual.getCantidad()) {
    ...
    private void guardarEstado(ArrayList<Diluidor> diluidores, ArrayList<Lote> lotes, int indiceLote) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(estadoFile, true))) {
            ...
            for (Lote lote : lotes) {
                sb.append(lote.getId()).append(".")
                        .append(lote.getFechaFin()).append(".")
                        .append(lote.getDiluidor()).append(","); // Separar atributos con punto
            }
        ...
        try {
            ...
            if (!lines.isEmpty()) {
                ...
                for (int i = 0; i < lotes.size(); i++) {
                    ...
                    if (fecha != null && !fecha.isEmpty()) {
                        lotes.get(i).setFechaFin(LocalDate.parse(fecha));
                    } else {
                        lotes.get(i).setFechaFin(null); // Manejar fechas nulas
                    }
                    lotes.get(i).setDiluidor(Integer.parseInt(loteParts[2]));
                }
    ...
    public void actualizarLotes(ArrayList<Lote> nuevosLotes) {
    ...
    private void calcularFechasInicio(Solucion solucion) throws PPGSchedulerException {
        for (Lote lote : solucion.getLotes()) {
            if (lote.getFechaFin() == null || lote.getDuracion() <= 0) {
            ...
            LocalDate fechaInicio = lote.getFechaFin().minusDays(lote.getDuracion());
            lote.setFechaInicio(fechaInicio); // Actualizar la fecha de inicio del lote
        }
    }
    ...
    public boolean ajustarSolucionesConHashMap(Solucion solucion) {
        ...
        for (HashMap.Entry<Integer, ArrayList<Lote>> entry : lotesPorDiluidor.entrySet()) {
            ...
            for (Lote lote : lotesDelDiluidor) {
                LocalDate fechaFin = lote.getFechaFin();
                LocalDate fechaNecesidad = lote.getFechaNecesidad();
     ...
     */
    private HashMap<Integer, ArrayList<Lote>> agruparLotesPorDiluidor(Solucion solucion) {
        HashMap<Integer, ArrayList<Lote>> lotesPorDiluidor = new HashMap<>();
        System.out.println("Agrupando lotes por diluidor...");

        for (Lote lote : solucion.getLotes()) {
            int diluidorId = lote.getDiluidor();
     ...
     */
    private boolean esAjusteValido(ArrayList<Lote> lotesDelDiluidor, Lote lote, LocalDate nuevaFechaInicio) {
        System.out.println("Verificando si el ajuste es válido para Lote ID: " + lote.getId() +
                ", Nueva Fecha Inicio: " + nuevaFechaInicio);

        for (Lote otroLote : lotesDelDiluidor) {
            if (!otroLote.equals(lote)) {
                LocalDate otroInicio = otroLote.getFechaInicio();
                LocalDate otroFin = otroLote.getFechaFin();
    ...
    private boolean puedeMoverLote(Lote lote, int dias) {
        LocalDate nuevaFechaInicio = lote.getFechaInicio();
        LocalDate nuevaFechaFin = lote.getFechaFin();
        ...
        lote.setFechaInicio(nuevaFechaInicio);
        lote.setFechaFin(nuevaFechaFin);
        return true;
    }
     ...
     */
    private void imprimirLotes(ArrayList<Lote> lotes) {
        System.out.println("Lotes:");
        for (int i = 0; i < lotes.size(); i++) {
            Lote lote = lotes.get(i);
            String informacionLote = "l" + (i + 1) +
                    " (Cantidad: " + lote.getCantidad() + ")" +
                    " (Duración: " + lote.getDuracion() + ")" +
                    " (Fecha fin: " + lote.getFechaFin().toString() + ")" +
                    " (Diluidor: " + lote.getDiluidor() + ")";
            ...
            for (Solucion solucion : soluciones) {
                ...
                for (Lote lote : solucion.getLotes()) {
                    writer.write(" - Lote ID: " + lote.getId() +
                            ", Cantidad: " + lote.getCantidad() +
                            ", Duración: " + lote.getDuracion() +
                            ", Fecha Fin: " + (lote.getFechaFin() != null ? lote.getFechaFin() : "No asignada") +
                            ", Fecha Necesidad : " + lote.getFechaNecesidad() +
                            ", Diluidor: " + (lote.getDiluidor() > 0 ? "Diluidor " + lote.getDiluidor() : "No asignado")
                            + "\n");
                }
    ...
    private void imprimirEstadoDiluidores() {
        ...
        for (Diluidor diluidor : diluidores) {
            ...
            for (Lote lote : lotes) {
                if (lote.getDiluidor() == diluidor.getId()) {
                    estado.append("->(l")
                            .append(lote.getId())
                            .append(" ")
                            .append(lote.getFechaFin() != null ? lote.getFechaFin().toString() : "Sin fecha")
                            .append(")");
                }
            }
            estado.append("\n");
